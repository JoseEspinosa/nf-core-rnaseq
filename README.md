...
This is a fork of the original [nf-core/rnaseq](https://github.com/nf-core/rnaseq) pipeline that has been adapted to perform the rnaseq analysis within the [BovReg](https://www.bovreg.eu/) project. On this fork, we added two subworkflows to the pipeline while maintaining all its original functionalities. In short, the `quantify_stringtie` subworkflow quantifies transcript expression using the de-novo annotation produced by `Stringtie`. The `annotate_feelnc` subworkflow also uses the de-novo StringTie annotation to predict lncRNAs.
